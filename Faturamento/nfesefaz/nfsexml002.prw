		...
		While !(cAliasSD2)->(Eof()) .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
			...
			If dbSeek(xFilial("SX5")+"60"+RetFldProd(SB1->B1_COD,"B1_CODISS"))
				...
				If !lNFeDesc  .And. nCont == 1
					If  cCodmun == "3300704-3156700"
						cNatOper := If(FindFunction('CleanSpecChar'),CleanSpecChar(AllTrim(SubStr(aSX5[1,4],1,55))),AllTrim(SubStr(aSX5[1,4],1,55))) + cNatOper
	    			Else
						cNatOper += If(FindFunction('CleanSpecChar'),CleanSpecChar(AllTrim(SubStr(aSX5[1,4],1,55))),AllTrim(SubStr(aSX5[1,4],1,55)))    			
	    			EndIf
	    	    ElseIf nCont == 1 
	    	        If cCodmun == "3300704-3156700"
						cDescrNFSe := If(FindFunction('CleanSpecChar'),CleanSpecChar(AllTrim(SubStr(aSX5[1,4],1,55))),AllTrim(SubStr(aSX5[1,4],1,55))) + cNatOper
	    			Else
						cDescrNFSe := If(FindFunction('CleanSpecChar'),CleanSpecChar(AllTrim(SubStr(aSX5[1,4],1,55))),AllTrim(SubStr(aSX5[1,4],1,55)))    			
	    			EndIf
	    	    EndIf 
    		EndIf
			//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
...
For Nx := 1 to Len(aProd)   
	...
	Else 
		cXml += '<ItemListaServico>'+ConvType2(aProd[Nx][24],5)+'</ItemListaServico>'   
	Endif
	If cCodMun $ "5201108-4104808-5103403-3524709-3300704-4313409-1400100-3156700-3205309-4308201-3513009" // Anápolis-GO, Cascavel-PR, Cuiabá-MT, Boa Vista-RR, Cotia-SP     
