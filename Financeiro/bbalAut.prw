...
Arquivo gerado automaticamente na pasta \Bankbal\.
...
User Function bbalAut(aParm)
	Local lSChed    := .F.
	Local aTabs     := {"SA1","SA6","CT2","CT1","SE5","CQ1"}
	Local aArea := FWGetArea()
	Default aParm   := {"01","0101"}
	
	if Select("SM0")<=0
		RpcSetEnv( aParm[1],aParm[2],,, "SIGAFIN","RELATORIO DE BANCO A GRAVAR NO FTP", aTabs,,,,)
		lSChed := .T.
	ENDIF

	Private dFrom := stod("")//Primeiro dia do mês atual
	Private dTo := sToD("")//Data atual
	...
	dFrom := FirstDate(date())
	dTo   := date()

	//Chama a função que gera o Excel
	fGeraExcel()
...
Criacao do arquivo Excel na funcao bbalAut
@author Marques Tec
@since 26/12/2025
/*/
...
Static Function fGeraExcel()
	...
	Local dData       := Date()
	Local cHora       := Time()
	Local cLocal      := "/Bankbal"
	Local cArquivo    := '/spool/bbalrel' + dToS(dData) + '_' + StrTran(cHora, ':' , '-' ) + '.rel'
	Local cSql        := ''
	Local cSqlSaldo    := ''
	Local nAtual      := 0
	Local aColunas    := {}
	...
	//Cria pasta no rootpath caso nao exista
	If ! ExistDir(cLocal)
		MakeDir(cLocal)
		conout("Pasta '" + cLocal + "' criada")
	EndIf
	...
	If ! cSaldo->(EoF())
		If ! cSe5->(EoF())
			...
			If oPrintXlsx:Activate(cArquivo)
				...
				lret := oPrintXlsx:ToXlsx()
				oPrintXlsx:DeActivate()
 
				cArquivo := StrTran(cArquivo, ".rel", ".xlsx")
 
				//if file(cArquivo)
				//	CpyS2T(cArquivo, 'C:\Temp\clientes\')
				//endif
				//E agora vamos abrir ele
				//cArquivo := ChgFileExt(cArquivo, '.xlsx')
				u_SFTPEnv(cArquivo,cLocal)
			EndIf

		Else
			conout('Não foi encontrado registros com os filtros informados!')
		EndIf
		cSe5->(DbCloseArea())
	Else
		conout('Não foi encontrado registros com os filtros informados!')
...
User Function SFTPEnv(cArq,cFTPDest)
	Local cEndereco	:=	allTrim( superGetMv( "ES_SFTPEND", , "ed122737.protheus.cloudtotvs.com.br" ) )
	Local cUsr		:=	allTrim( superGetMv( "ES_SFTPUSR", , "ftp_C9MJO6_prod" ) )
	Local cPass		:=	allTrim( superGetMv( "ES_SFTPPSW", , "gMFVWFI4UtW7zGRF0J3xbEGO" ) )
	Local nPorta	:=	superGetMv( "ES_SFTPPRT", , 2323 )
	Local _cNomeArqu:=	cArq
	local nStatus
	local cError 	:= ''
	Local _cArqFtp	:=	cFTPDest+"/"+_cNomeArqu


	If File(cArq)

		nStatus := SFTPUpld1(cArq, _cArqFtp, cEndereco+':'+CVALTOCHAR(nPorta), cUsr, cPass,@cError)
