...
Do Case
    ...
    Case _cTipo == "PP001"	//  Agencia e Conta Corrente Favorecido
       ...
       If sa2->a2_banco == "341"
          ...
          _cRetorno += strzero(val(substr(_cConta,6,5)),5)+" "+substr(sa2->a2_xdvcta,1,1)                               
       Else
          _cRetorno := strzero(val(substr(sa2->a2_agencia,1,5)),5)+" "
          _cRetorno += strzero(val(substr(_cConta,1,12)),12)
          If len(alltrim(sa2->a2_xdvcta)) > 1
            _cRetorno += strzero(val(substr(sa2->a2_xdvcta,1,2)),2)
          Else
             _cRetorno += " "+substr(sa2->a2_xdvcta,1,1)    	
          EndIf                                     
       EndIf
    ...
    Case _cTipo == "PT001"	                           
         ...
         If SEA->EA_MODELO == "16"    
            ...
            _cRetorno += If(!Empty(SE2->E2_XCRDARF),STRZERO(Val(SE2->E2_XCRDARF),4),STRZERO(Val(SE2->E2_CODRET),4))
  
            // Posicao 024 a 024: Tp Inscricao  1-CPF /  2-CNPJ               
           If !Empty(SM0->M0_CGC)
               _cRetorno += Iif (len(alltrim(SM0->M0_CGC))>11,"2","1")
           Else               
              _cRetorno += "2"       
           EndIf
            ...
            _cRetorno += GRAVADATA(LASTDAY(SE2->E2_XDTRIB),.F.,5) 
            
            // Posicao 047 a 063: Referencia   
            _cRetorno +=  "00000000000000000"
         ...
         ElseIf (SEA->EA_MODELO == "17")   
            ...
            _cRetorno +=  STRZERO(Val(SE2->E2_XCODINS),4)  
                                                 
            // Posicao 024 a 029: Competencia   MMAAAA  
            _cRetorno += STRZERO(MONTH(SE2->E2_XDTRIB),2)+STRZERO(YEAR(SE2->E2_XDTRIB),4)
            //_cRetorno += FORMULA("261")        
           ...
           EndIf
            ...
            _cRetorno += STRZERO((SE2->E2_SALDO-SE2->E2_XVLENT)*100,14)                                       
            
            // Posicao 058 a 071: Valor Outras Entidades
            _cRetorno += STRZERO((SE2->E2_XVLENT)*100,14)                
            
            // Posicao 072 a 085: Multa        
            _cRetorno += STRZERO(SE2->E2_ACRESC*100,14)                      
            
            // Posicao 086 a 099: Valor Total (Principal + Multa)
            _cRetorno += Strzero((SE2->E2_SALDO+SE2->E2_ACRESC+SE2->E2_JUROS)*100,14)              
         ...
         ElseIf  SEA->EA_MODELO == "18"   
            ...
            // Posicao 106 a 119: Valor Total (Principal + Multa + Juros)
            _cRetorno += STRZERO((SE2->E2_SALDO+SE2->E2_ACRESC+SE2->E2_E_JUROS)*100,14)           
         ...
         ElseIf SEA->EA_MODELO == "35" 
            ...
            // Posicao 020 a 023: Codigo da Receita  
            _cRetorno +=  IF(!EMPTY(SE2->E2_XCODINS),SE2->E2_XCODINS,"0115")      
            ...
            _cRetorno += SE2->E2_XLINDIG        
            
            // Posicao 087 a 102: Identificador do FGTS
            _cRetorno += Strzero(Val(SE2->E2_XCNPJ),16)                                    
