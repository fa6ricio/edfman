...
WsRestful EDNFCANC Description 'API de Notas Canceladas.'

	WSDATA Page     AS INTEGER OPTIONAL
	WSDATA PageSize AS INTEGER OPTIONAL

	WsMethod GET 1 Description 'Retorna notas canceladas.' WSSYNTAX "/v1/outbound-docs" PATH '/'
	WsMethod PUT 2 Description 'Atualiza Nota Cancelada.' WSSYNTAX "/v1/outbound-docs" PATH '/'

END WsRestful

// Retorna todos os pedidos de venda
WsMethod GET 1 WsRestful EDNFCANC
...
BEGIN SEQUENCE
	...
	While (cAliasMain)->(!Eof()) .And. nRead < (nWanted + 1)
		...
		If nRead >= nWanted
			lHasNext := .T.
			cNextCursor := cValToChar((cAliasMain)->REC_SF2) // cursor do "prÛximo" comeÁo
			Exit
		EndIf
		...
		oNf["NfDoc"]          := IIF(EMPTY(AllTrim((cAliasMain)->C5_PEDEXP)),AllTrim((cAliasMain)->C5_NUM),AllTrim((cAliasMain)->C5_PEDEXP))  
...
WSMETHOD PUT 2 WSRESTFUL EDNFCANC
...
cQuery += " AND SF3.F3_CODRSEF  = '101' "
cQuery += " AND SF3.F3_CFO IN ('6505','5505')"
...
Static Function DateToISO(uParam, cTime)
	...
	// Verifica se √© Data mesmo ou String que veio do SQL
	If ValType(uParam) == "D"
		cDate := DtoS(uParam)
