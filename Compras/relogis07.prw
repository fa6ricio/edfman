...
User Function Relogis7()
/*
ÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜ
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºPrograma  ³RELOGIS07 ºAutor  ³Renato Bandeira     º Data ³  04/19/12   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³ Relaciona as Notas de Entrada, excecao acucar.             º±±
±±º          ³                                                            º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
...
cReferen := ""
cContrato:= ""
cTes     := "005"     // Alexandre Santos - 24/07/2013 - De Para da TES
// cTes     := "005"
cNFMAE   := space(09)
cContra  := "               "
cPedido  := "      "
...
@180,20 Button oBtnOk     Prompt "&Consulta"       Size 30,15 Pixel Action (U_gerlog07(), oDlg:End()) Of oDlg
...
User Function Gerlog07 // gera consulta para negociação com o fornecedor.
Local aDados := {}
Local aCampos:= {}
Local aCabec := {}
...
cQuery+=" F1_XNOMFOR AS FORNECEDOR, (C7_VLFINAL*D1_QUANT) AS VL_USD, C7_NRMEDIA AS MEDIA, C7_VLFINAL AS VL_FINAL, D1_LOCAL, D1_COD " //Z3_PREMIO4 " // Alexandre Santos 19/07/2013
//cQuery+=" F1_XNOMFOR AS FORNECEDOR, (C7_VLFINAL*D1_QUANT)/20 AS VL_USD, C7_NRMEDIA AS MEDIA, C7_VLFINAL AS VL_FINAL, D1_LOCAL " //Z3_PREMIO4 " // Alexandre Santos 19/07/2013
...
AAdd(aCabec, "Vl.Unitario")
AADD(ACabec, "Vl.Total")
...
IF !TRB->(EOF())                      
	...
	wtons:=TRB->TONS*nFator               // Alexandre Santos - 19/07/2013
...
While !TRB->(EOF())
	...
	IF WCONTRA<>&(aCampos[2]) .OR. WMEDIA<>&(aCampos[14]) .OR. TRB->(EOF())
		...
		AAdd(aDados ,{" "," "," "," "," ",nvltotal,"", "",(nvltotal)/20,"","",WTONUSD,"","","",wtotnf,"",wsomprem4})
...
AAdd(aDados ,{" "," "," "," ","Total: ",nvltotalg," ","Tons.: ",(nvltotalg)/20,"","",WTONUSDG,"","","",wtotnf,"",wsomprem4g})
...
While !TRB->(EOF())
	...
	*/
	...
	TRB->(dbSkip())
	/*/ RENATO EM 19/4/12 - QUEBRA NAO EH NECESSARIA
...
/*/
Renato em 19/4/12 - incluir select do se2 , rateio e nao rateio

SELECT E2_NUM,E2_PARCELA,E2_FORNECE,E2_EMISSAO,CV4_VALOR AS E2_VALOR,CV4_CLVLDB as E2_CLVLDB
 FROM SE2010 SE2 
 JOIN CV4010 CV4 ON CV4_FILIAL+CV4_DTSEQ+CV4_SEQUEN = E2_ARQRAT
 WHERE SE2.D_E_L_E_T_ <> '*' AND E2_RATEIO = 'S' AND E2_ARQRAT <> ' '
 AND LEFT(E2_ORIGEM,7) <> 'MATA100'

UNION ALL

SELECT E2_NUM,E2_PARCELA,E2_FORNECE, E2_EMISSAO,E2_VALOR,E2_CLVLDB
 FROM SE2010 SE2 
