...
/*
Rotina customizada EDR026.prw criada com base no fonte EECPEM26.PRW
Alteração 02/10/14
habilitar edição do campo: Nº Memorando exportação
*/

/*
09/02/15 - Luis Felipe Nascimento - o item exportado não estava sendo demonstrado porque o campo EE9_FATIT nao continha o 
número do item. Como os faturamentos são sobre um produto por vez, ou seja, não e

*/

User Function EDFR026()
...
//DFS - 02/10/12 - Inclusão de picture de acordo com o que foi digitado nas configurações do embarque
Private cPictPreco := "999,999,999.99999"//+cPictDecPrc //Luiz Pereira 15/07/15
Private cPictPeso  := "9,999,999.99999"  //+cPictDecPes //Luiz Pereira 15/07/15
Private cPictQtde  := "9,999,999.99999"  //+cPictDecQtd //Luiz Pereira 15/07/15
...
Private lCposMemExp := GetMv("MV_AVG0174",,.F.) .And. ChkFile("EYY");
                                                .And. EYY->(FieldPos("EYY_NROMEX")) # 0 ;
                                                .And. EYY->(FieldPos("EYY_DTMEX" )) # 0
...
Begin Sequence
   ...
   If Upper(FunName()) <> "EECAE100" .And. Upper(FunName()) <> "AA100DOC" 
      ...
      Define MsDialog oDlg Title STR0001 From 7,10 To 15,60 Of oMainWnd //"Memorando de Exportação"
...
Local nPos,;
      ...
      nQuantidade:= 0
Local lRet:= .F.
Local lAVG0174 := GetMv("MV_AVG0174",, .F.)
...
Begin Sequence   
   ...
   While EEM->(!Eof()) .And.;
      ...
      If EEM->EEM_TIPONF == EEM_SD  //Se nota fiscal de saída
                         //nota fiscal , modelo        , série         , data, Chave
         AAdd(aNotasExp, {EEM->EEM_NRNF, EEM->EEM_MODNF, EEM->EEM_SERIE, EEM->EEM_DTNF,Posicione("SF2", 1, xFilial("SF2") + AVKEY(EEM->EEM_NRNF,"F2_DOC")+AVKEY(EEM->EEM_SERIE,"F2_SERIE"), "F2_CHVNFE") })      //ENS 17/05/16
      EndIf
      EEM->(DBSkip())
   EndDo
   ...
   While EE9->(!Eof()) .And.;
      ...
      Else
         ...
         If lCposMemExp
            aOrdEYY := SaveOrd({"EYY"})
            EYY->(DbSetOrder(3))
            EYY->(DbSeek(xFilial("EYY") + EEC->EEC_PREEMB + EE9->EE9_PEDIDO + AvKey(EE9->EE9_SEQUEN,"EYY_SEQUEN")))   // GFP - 05/03/2014
            cMemoExpo := EYY->EYY_NROMEX
            RestOrd(aOrdEYY)
         EndIf
      EndIf
      ...
      If (nPos:= AScan(aRe, {|x| x[1] == cFabricante+If(lCposMemExp,cMemoExpo,"")})) == 0
         AAdd(aRe, {cFabricante+If(lCposMemExp,cMemoExpo,"")})
         nPos:= Len(aRe)
      EndIf
      ...
      */
      ...
      If (nPos:= AScan(aProdutos, {|x| x[1] == cFabricante+If(lCposMemExp,cMemoExpo,"")})) == 0
         AAdd(aProdutos, {cFabricante+If(lCposMemExp,cMemoExpo,"")})
         nPos:= Len(aProdutos)
      EndIf
      ...
      If IsIntFat() .And. !Empty(EE9->EE9_NF)
         ...
         SD2->(DBSeek(xFilial("SD2") + AvKey(EE9->EE9_NF, "D2_DOC") + AvKey(EE9->EE9_SERIE, "D2_SERIE") +;
                                  AvKey(EEC->EEC_IMPORT, "D2_CLIENTE") + AvKey(EEC->EEC_IMLOJA, "D2_LOJA") +;
                                  AvKey(EE9->EE9_COD_I, "D2_COD")))  // 09/02/15 - Luís Felipe Nascimento 
//                                  AvKey(EE9->EE9_COD_I, "D2_COD") + AvKey(StrZero(Val(EE9->EE9_FATIT),2),"D2_ITEM")))  // GFP - 05/03/2014
         
         // GFP - 05/03/2013 - Looping para tratamento caso exista o mesmo item mais de uma vez na mesma NF.
         Do While SD2->(!Eof()) .AND. SD2->D2_FILIAL == xFilial("SD2") .AND. SD2->D2_DOC == EE9->EE9_NF .AND.;
                                      SD2->D2_SERIE == EE9->EE9_SERIE .AND. SD2->D2_CLIENTE == EEC->EEC_IMPORT .AND.;
                                      SD2->D2_LOJA == EEC->EEC_IMLOJA .AND. SD2->D2_COD == EE9->EE9_COD_I // .AND.; // 09/02/15 - Luís Felipe Nascimento 
//                                      SD2->D2_ITEM == AvKey(StrZero(Val(EE9->EE9_FATIT),2),"D2_ITEM")  // GFP - 05/03/2014 // // 09/02/15 - Luís Felipe Nascimento 
      
            SC6->(DBSetOrder(1)) //C6_FILIAL + C6_NUM + C6_ITEM + C6_PRODUTO
            SC6->(DBSeek(xFilial("SC6") + SD2->D2_PEDIDO + SD2->D2_ITEMPV + AvKey(EE9->EE9_COD_I, "C6_PRODUTO")))
      ...
      If AScan(aFabricante, {|x| x[1] == cFabricante+If(lCposMemExp,cMemoExpo,"")}) == 0

         AAdd(aFabricante, {cFabricante+If(lCposMemExp,cMemoExpo,""),; //1. fabricante
      ...
      /*---------------------
        ...
        -------------------------------------------*/
      //Se não encontrar o fabricante, o inclui e retorna a nova posição (última).
      If (nPos:= AScan(aNfRemessa, {|x| x[1] == cFabricante+If(lCposMemExp,cMemoExpo,"")})) == 0
         AAdd(aNfRemessa, {cFabricante+If(lCposMemExp,cMemoExpo,"")})
         nPos:= Len(aNfRemessa)
      EndIf
      
      //If !Empty(cNfRem)  // Nopado por GFP - 24/01/2013
         //aNfRemessa[cFabricante], {Nota fiscal de remessa, Modelo, Série, Emissão, Quantidade, Unidade, NCM, Descrição}
         //Não serão armazenadas notas fiscais repetidas
         //If Len(aNfRemessa[nPos]) == 1

            
      If IsIntFat() .And. lAVG0174   // GFP - 24/01/2013 - Sistema deve exibir todas as NFs vinculadas ao embarque.
         
          
         
          //-------------------------------------------------------------
          If EYY->(DBSeek(xFilial() + EE9->EE9_PREEMB))  //ENS 30/05/16  
            ...
            While EYY->(!Eof()) .And.;
                  ...
                  //Alterado pois estava trazendo os dados do fabricante incorreto para as NFs de remessa

               //If EYY->EYY_FORN+EYY->EYY_FOLOJA == EE9->EE9_FABR+EE9->EE9_FALOJA 
               If AllTrim(EYY->EYY_SEQUEN) == AllTrim(EE9->EE9_SEQUEN)  .AND. cMemoExpo == EYY->EYY_NROMEX

                  //D1_FILIAL + D1_COD + D1_DOC + D1_SERIE + D1_FORNECE + D1_LOJA
                  If SD1->(DBSeek(xFilial() + AvKey(EE9->EE9_COD_I, "D1_COD") +;
                                  AvKey(EYY->EYY_NFENT, "D1_DOC") +;
                                  AvKey(EYY->EYY_SERENT, "D1_SERIE") +;
                                  AVKEY(/*cFabricante*/EYY->EYY_FORN,"D1_FORNECE") +;
                                  AVKEY(EYY->EYY_FOLOJA,"D1_LOJA")))   
                                  //AVKEY(EYY->EYY_FOLOJA,"D1_FORNECE")))  // GFP - 05/03/2013  
                         
        //----------------------------------------------------------------------- 
                                 

                    /* AAdd(aNfRemessa[nPos], {SD1->D1_DOC,;
                                    cModNfRem,;
                                    SD1->D1_SERIE,;
                                    SD1->D1_EMISSAO,;
                                    EYY->EYY_QUANT,;
                                    SD1->D1_UM,;
                                    Posicione("SB1", 1, xFilial("SB1") + SD1->D1_COD, "B1_POSIPI"),;
                                    Posicione("SB1", 1, xFilial("SB1") + SD1->D1_COD, "B1_DESC");
                                    }) //LGS-21/02/2014 */ 
                                    
                                    
                                    
                    AAdd(aNfRemessa[nPos], {SD1->D1_DOC,;       //ENS 17/05/16
                                    cModNfRem,;
                                    SD1->D1_SERIE,;
                                    SD1->D1_EMISSAO,;
                                    EYY->EYY_QUANT,;
                                    ...
                                    Posicione("SB1", 1, xFilial("SB1") + SD1->D1_COD, "B1_POSIPI"),;
                                    Posicione("SB1", 1, xFilial("SB1") + SD1->D1_COD, "B1_DESC"),;
                                    Posicione("SF1", 1, xFilial("SF1") + SD1->D1_DOC+SD1->D1_SERIE, "F1_CHVNFE");  //ENS 17/05/16
                                    }) 
      ...
      EndIf  
         ...
         //DFS - 24/08/12 - Retirado tratamento que impedia a impressão do segundo item se tivesse mesma nota fiscal de remessa.
      ...
      If (nPos:= AScan(aConhTransp, {|x| x[1] == cFabricante+If(lCposMemExp,cMemoExpo,"")})) == 0
         AAdd(aConhTransp, {cFabricante+If(lCposMemExp,cMemoExpo,"")})
      EndIf
   ...
   If lCposMemExp .AND. Len(aFabricante) > 0 //DFS - 06/10/12 - Apenas adicionar valor ao campo se o array for maior que zero.
      cNumMemo := Substr( aFabricante[1][1], Len(SA2->A2_COD)+Len(SA2->A2_LOJA)+1 )        
   EndIf
...
Begin Sequence
   ...
   Define MsDialog oDlg Title STR0001 + cTitulo From 7, 0 To 35, 100 Of oMainWnd //Memorando de Exportação x de n - Razão Social do Produtor/ Fabricante
   
      oFld:= TFolder():New(15, 3, {"Exportador",;  //Exportador
                                   "Dados da exportação",;  //Dados da exportação
                                   "Discriminação dos produtos exportados",;  //Discriminação dos produtos exportados
                                   "Remetente com fim específico de exportação",;  //Remetente com fim específico de exportação
                                   "Dados dos documentos fiscais de remessa",;  //Dados dos documentos fiscais de remessa
                                   "Dados dos conhecimentos de transporte",;  //Dados dos conhecimentos de transporte
                                   "Dados do transportador",;  //Dados do transportador
                                   "Representante legal do exportador",;  //Representante legal do exportador
                                   "Informações Adicionais"},; //Informações Adicionais
                           {"F1", "F2", "F3", "F4", "F5", "F6", "F7", "F8", "F9"}, oDlg,,,, .T., .F., 390, 193)
      ...
      TGet():New(xLin2(nLin), nCol + 80, BSetGet(cNumMemo), aFld[nFld_a], 80, 08, "@!",,,,,,, .T.,,,,,,,.F./*lCposMemExp*/) 
      ...
      xLin1(nLin)
      oGetDb_b:= MsGetDb():New(nlin, aPos[2] , aPos[3]-75, aPos[4],;  // Posições da Tela
      ...
      aAltFld_b2:= {}
      oGetDb_b2:= MsGetDb():New(aPos[3]-75, aPos[2], aPos[3]-11, aPos[4],; // Posições da Tela
                                                                       4,; // Tipo de acordo com o aRotina(Inclusão, Alteração, etc.)
                                                                        ,; // cLinhaOk
                                                                        ,; // cTudoOk
                                                                        ,; // cIncrementa
      ...
      oGetDb_c:= MsGetDb():New(aPos[1], aPos[2] ,aPos[3]-11, aPos[4],; // Posições da Tela
                                             ...
                                             "U_PEM26CP2('Work2')",; // Função executada na validação do campo
                                                                    ,; // Reservado
                                                                 .F.,; // Define se as linhas poderão ser incluídas
      ...
      */
      ...
      nLin:= 10
      nCol:= 5
      TSay():New(xLin1(nLin), nCol     , {|| "Informações Adicionais"}, aFld[nFld_i],,,,,, .T.) //Informações Adicionais
      TGet():New(xLin2(nLin), nCol + 72, BSetGet(cInfCompl), aFld[nFld_i], 230, 08, "@!",,,,,,, .T.)      
      
      //DFS - 31/10/12 - Inclusão de Nr e Data da DSE.
      TSay():New(xLin1(nLin), nCol     , {|| "Número DSE"}, aFld[nFld_i],,,,,, .T.) //Número DSE
      TGet():New(xLin2(nLin), nCol + 72, BSetGet(cNumDSE), aFld[nFld_i], 230, 08, "@!",,,,,,, .T.)    
      
      TSay():New(xLin1(nLin), nCol     , {|| "Data DSE"}, aFld[nFld_i],,,,,, .T.) //Data DSE
...
Begin Sequence
      ...
      For nCont:= 1 To 3 //3 //número de vias - alteração conforme convênio 84/09
         ...
         If lCposMemExp  // ENS - 01/06/2016
            cNumMemo := Substr( aFabricante[nFabr][1], Len(SA2->A2_COD)+Len(SA2->A2_LOJA)+1 )
         EndIf
         ...
         If nCont == 1
            ...
            For nCont2:= 1 To Len(aNotasExp)
               ...
               */
               ...
               DETAIL_P->AVG_C01_60:= AllTrim(aNotasExp[nCont2][5])    // ENS 17/05/16
            ...
            For nCont2:= 2 To Len(aNfRemessa[nFabr])
               ...
               */
               ...
               //DFS - 02/10/12 - Inclusão de picture de acordo com o que foi digitado nas configurações do embarque
               DETAIL_P->AVG_C06_20:= AllTrim(Transf(aNfRemessa[nFabr][nCont2][5], cPictQtde))
               ...
               DETAIL_P->AVG_C02_60:= AllTrim(aNfRemessa[nFabr][nCont2][9]) //ENS 17/05/16
               DETAIL_P->(MsUnlock())
      ...
      //Gravação do número do memorando de exportação na tabela EXL
      ...
      EndIf
        ...
        *****************************************************************        
        *** Correção da incrementação da numeração do memorando
        *** Luiz Pereira - 19/08/2015
        *****************************************************************
//           cNumMemo := Strzero(Val(cNumMemo)+1,Len(alltrim(cNumMemo)))   nopado correção efetuada em outro local, pois desta forma ele não obedecia o numero do memorando ENS 01/06/2016  
        *****************************************************************
...
Begin Sequence
   ...
   */
   ...
   cAlias:= "Work1"
   oAliasTrb:= FwTemporarytable():New(cAlias,aEstrutura)
   oAliasTrb:Create()

   //Armazena os arquivos criados para exclusão
   AAdd(aArq, {cAlias, cAlias})
   ...
   /*Notas fiscais do exportador. Serão apenas exibidas.
     ...
     4. Data
   */
   aEstrutura:= { ;//ENS 17/05/16
                 {"WK_NFEXP"  , "C", AvSx3("D1_DOC", AV_TAMANHO)    , 0},;
                 {"WK_MODNFEX", "C", 3                              , 0},;
                 {"WK_SERNFEX", "C", AvSx3("D1_SERIE", AV_TAMANHO)  , 0},;
                 {"WK_DTNFREX", "D", AvSx3("D1_EMISSAO", AV_TAMANHO), 0},;
                 {"WK_FLAG"   , "L", 1                              , 0},;
                 {"WK_CHAVENF", "C", AvSx3("F1_CHVNFE", AV_TAMANHO)  , 0}}

   cAlias:= "Work4"
   oAliasTrb2:= FwTemporarytable():New(cAlias,aEstrutura)
   oAliasTrb2:Create()
   ...
   For nCont:= 1 To Len(aNotasExp)
      ...
      (cAlias)->WK_CHAVENF:= aNotasExp[nCont][5]
   Next
   (cAlias)->(DBGoTop())   
   ...
   aEstrutura:= {{"WK_COD"   , "C", AvSx3("EE9_COD_I" , AV_TAMANHO), 0                              },;
                 ...
                 {"WK_DESC"  , "C", AvSx3("C6_DESCRI" , AV_TAMANHO), 0                              },;
                 {"WK_QTD"   , "N", AvSx3("EE9_SLDINI", AV_TAMANHO), 5								},; //Luiz Pereira 15/07/15
                 {"WK_UNID"  , "C", AvSx3("EE9_UNIDAD", AV_TAMANHO), 0                              },;
                 {"WK_VLUNIT", "N", AvSx3("EE9_PRECO" , AV_TAMANHO), 5								},; //Luiz Pereira 15/07/15
                 {"WK_VLTOT" , "N", AvSx3("EE9_PRCTOT", AV_TAMANHO), AvSx3("EE9_PRCTOT", AV_DECIMAL)},;
                 {"WK_FLAG"  , "L", 1                              , 0                              }}


   cAlias:= "Work2"
   oAliasTrb3:= FwTemporarytable():New(cAlias,aEstrutura)
   oAliasTrb3:Create()
   ...
   */
   aEstrutura:= {;//ENS 17/05/16
                 {"WK_NFREM"  , "C", AvSx3("D1_DOC", AV_TAMANHO)    , 0},;
                 {"WK_MODNFRE", "C", 3                              , 0},;
   ...
   cAlias:= "Work3"
   oAliasTrb4:= FwTemporarytable():New(cAlias,aEstrutura)
   oAliasTrb4:Create()
   ...
   For nCont:= 2 To Len(aNfRemessa[nFabr])
      ...
      (cAlias)->WK_MODNFRE:= "55" //aNfRemessa[nFabr][nCont][2] // alterado Fernando 06/07
      ...
      (cAlias)->WK_CHAVENF:= aNfRemessa[nFabr][nCont][9] //17/05/16
   Next
   (cAlias)->(DBGoTop())
   ...
   */
   ...
   cAlias:= "Work5"
   oAliasTrb5:= FwTemporarytable():New(cAlias,aEstrutura)
   oAliasTrb5:Create()
...
Begin Sequence
   ...
   AAdd(aHeader, {"CHAVE",; //CHAVE         ENS 17/05/16
                  "WK_CHAVENF",;
                  X3Picture("F1_CHVNFE"),;
