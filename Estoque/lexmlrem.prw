...
Private cLocFile   := cGetFile( '*.xml' , ' Importação NF_REMESSA(xml)', 1, 'C:\', .F., nOR( GETF_LOCALHARD, GETF_LOCALFLOPPY, GETF_RETDIRECTORY ),.T., .T. )
Private cLocMain   := "C:\IMPORTACAO_XML\"
Private cLocSubMain:= cLocMain+"NF_REMESSA\"
...
{ "NF_CONTRA"  , "C" , 09 , 0 , "CONTRATO"      ,"PadR(Left(@@,09),09)"                    },;
{ "NF_PERIODO"  , "C" , 03 , 0 , "PERIODO"      ,"PadR(Left(@@,03),03)"                    },;
...
lMAECONTRA := .f.  //INDICA SE NOTA MAE/CONTARTO INFORMADO EXISTEM
...
For nRegXml := 1 To nRegs
	...
	For X_X := 1 to n_tam_det
		...
		*/
		...
		If !X_futura $ "922/501/503/494" .AND. TYPE("OXML:_NFEPROC:_NFE:_INFNFE:_TRANSP:_TRANSPORTA:_CNPJ:TEXT")<>"U"
		...
		*/
		...
		IF /*SA2->A2_COD<>"000167" .AND.*/ TYPE("OXML:_NFEPROC:_NFE:_INFNFE:_infAdic:_infCpl:TEXT")<>"U"
			...
			*'Verifica se nota de remessa possui nota mãe-----------------------------------------------------------------------'*
			...
			nQtd := U_EDFFATOR(SB1->B1_COD) * (iif(n_tam_det>1,VAL(OXML:_NFEPROC:_NFE:_INFNFE:_DET[X_X]:_PROD:_QTRIB:TEXT),VAL(OXML:_NFEPROC:_NFE:_INFNFE:_DET:_PROD:_QTRIB:TEXT)))
			...
			If "NOTA FISCAL DE VENDA" $ OXML:_NFEPROC:_NFE:_INFNFE:_infAdic:_infCpl:TEXT
				...
				at("NOTA FISCAL DE VENDA", OXML:_NFEPROC:_NFE:_INFNFE:_infAdic:_infCpl:TEXT)+32,2);
				,".","")
				...
				If dbseek(xFILIAL("SF1")+cnfmae+cSERImae)
					...
					DBSELECTAREA("SC7")
					DBSETORDER(4)
					If DBSEEK(xFILIAL("SC7")+SB1->B1_COD+xPEDIDO).And. !Empty(xPEDIDO)
						IF !SC7->(EOF())
							RecLock("SC7",.F.)
			...
			ELSE
				...
				lMAECONTRA := U_LEINFCPL(OXML:_NFEPROC:_NFE:_INFNFE:_infAdic:_infCpl:TEXT,@cnfmae,@cSERImae,@cCONTRA,@cPERIODO)
				...
				IF LMAECONTRA == .T.
					...
					IF !EMPTY(CCONTRA)
						...
						DBSELECTAREA("SC7")
						DBSETORDER(23)
						If DBSEEK(xFILIAL("SC7")+PADR(ALLTRIM(CCONTRA),TAMSX3("C7_CONTRAT")[1]) )
							
							Do While !SC7->(EOF()).AND. SC7->C7_CONTRAT=PADR(ALLTRIM(CCONTRA),TAMSX3("C7_CONTRAT")[1])
								
								If SC7->C7_CONTRAT=PADR(ALLTRIM(CCONTRA),TAMSX3("C7_CONTRAT")[1]) .AND. ;
									SC7->C7_XPERIOD=PADR(ALLTRIM(cPERIODO),TAMSX3("C7_XPERIOD")[1]) .AND. ;
									SC7->C7_COD=SB1->B1_COD
		...
		*'Yttalo P Martins-INICIO-------------------------------------------------------------------------------------------------'*
		...
		if !lFor .AND. !lProd .AND. !lTransp .OR. lMAECONTRA == .T.
			*'Yttalo P Martins-FIM----------------------------------------------------------------------------------------------------'*
			...
			(cAliasTrb)->TES        := cTES     //SF4->F4_CODIGO
			...
			(cAliasTrb)->TRANSP     := If( X_futura$"922/501/494","", SA4->A4_COD)
		...
		Else
			...
			*/
			...
			If !SF1->(EOF())
				...
				msunlock()
				U_SV_IncNt(SF1->F1_SERIE, SF1->F1_DOC, SF1->F1_FORNECE, SF1->F1_LOJA, SF1->F1_CONTRA)
			Endif
...
User function LEINFCPL(xcmemo,cnfmae,cSERImae,cCONTRA,cPERIODO)

Local oDlg, oMemo, cMemo := ""
Local XCONTRAT := SPACE(15)
Local XCPER   := SPACE(10)
...
@ 010,190 MSGET XCONTRAT Picture PesqPict("SZ3","Z3_CONTRA") Valid (XCnf:=SPACE(9),XCPER:=SPACE(10),dlgRefresh(oDlg)) OF oDlg PIXEL
@ 003,020 SAY "Período:"
@ 040,190 MSGET XCPER Picture PesqPict("SZ3","Z3_PERIODO") OF oDlg PIXEL
...
cPERIODO := iif(EMPTY(XCPER),"",PADR(ALLTRIM(XCPER),TAMSX3("Z3_PERIODO")[1]) )
...
SZD->ZD_QTDNFRE := nVOLUME
//SZD->ZD_QTDREC := nVOLUME
SZD->ZD_QTDMAE := XVOLMAE
SZD->ZD_SALDO := nVOLUME
